...
use crate::{
    ...
    protocol::Message,
    registry::{RegistryError, SubscriptionSlot},
    transport::{
        create_channel,
        MessageSender,
        in_memory::{
...
impl BrokerClient {
    ...
    pub(crate) fn new(broker: Arc<Broker>, config: ClientConfig, client_address: String) -> Result<Self, RegistryError> {
        ...
        let reply_slot = broker.registry.register(client_address.clone(), sender)?;

        let reply_subscriber = Subscriber::new(receiver, client_address.clone());
        let reply_handle = SubscriptionHandle::new(
        ...
        //let reply_slot = reply_subscriber.slot();
    ...
    /// - `None` (default): Exclusive binding. Fails if address is occupied.
    /// - `Some(LoadBalanced)`: Adds to a Round-Robin group. Fails if address 
    ///   is registered as Exclusive or Broadcast.
    /// - `Some(Broadcast)`: Adds to a Fan-out group. Fails if address 
    ///   is registered as Exclusive or LoadBalanced.
    pub fn subscribe(
        &self,
        address: String,
        ...
        let slot = self.broker.registry.register(address.clone(), sender)?;

        let subscriber = Subscriber::new(receiver, address.clone());
...
mod tests {
    ...
    async fn test_in_memory_message_delivery_in_only() {
        ...
        let (mut subscriber, _handle) = client.subscribe(target_address.clone(), subscriber_config).expect("Subscription should succeed");
    ...
    async fn test_multi_recipient_routing() {
        ...
        for addr in &addresses {
            let (subscriber, handle)  = client
                .subscribe(addr.to_string(), subscriber_config.clone())
    ...
    async fn test_dynamic_registration() {
        ...
        // Registration
        let (mut subscriber, handle) = client.subscribe("arcella:test".to_string(), subscriber_config).expect("Subscription should succeed");
...
async fn test_subscription_cleanup_and_re_registration() {
        ...
        let (_sub1, hdl1) = client.subscribe(addr.to_string(), subscriber_config.clone()).expect("First subscription should succeed");
        ...
        let (mut sub3, _hdl3) = client.subscribe(addr.to_string(), subscriber_config).expect("Subscription after drop should succeed");
